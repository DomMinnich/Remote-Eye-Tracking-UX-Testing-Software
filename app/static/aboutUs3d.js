...
document.getElementById('enableWallpaperMovement').checked = config.enableWallpaperMovement;
...
document.getElementById('enableWallpaperMovement').addEventListener('change', (event) => {
...
document.getElementById('orbitFocalDistance').addEventListener('input', (event) => {
...
const teamMembers = [
    { name: "Sulaiman Hussain", description: "Full Stack Developer", imageUrl: "static/pic.jpg", linkUrl: "https://example.com/sulaiman" },
    { name: "Dominic Minnich", description: "Team Leader\nFull Stack Developer", imageUrl: "static/pic.jpg", linkUrl: "https://github.com/DomMinnich" },
    { name: "Kyle Benich", description: "Full Stack Developer", imageUrl: "static/pic.jpg", linkUrl: "https://example.com/kyle" },
    { name: "Logan Smith", description: "Full Stack Developer", imageUrl: "static/pic.jpg", linkUrl: "https://example.com/logan" },
];
...
teamMembers.forEach((member, index) => {
    const panel = createInfoPanel(member.name, member.description, member.imageUrl, member.linkUrl);
...
function updatePanelVisibility() {
    panels.forEach((panel) => {
        ...
        if (visible && !panel.isVisible) {
            ...
            new TWEEN.Tween(panel.scale)
                .to({ x: panel.userData.originalScale.x * 6, y: panel.userData.originalScale.y * 6, z: panel.userData.originalScale.z * 6 }, 200)
                .start();
        } else if (!visible && panel.isVisible) {
            console.log(`Panel ${panel.name} is now invisible`);
            panel.isVisible = false;
            new TWEEN.Tween(panel.scale)
                .to({ x: panel.userData.originalScale.x, y: panel.userData.originalScale.y, z: panel.userData.originalScale.z }, 200)
                .start();
        }
    });
}
